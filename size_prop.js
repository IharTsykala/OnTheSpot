...
if (
  ...
  $text = true;
  jQuery(".ProductForm__Option option").each(function (index) {
    if (
      jQuery(this).text().trim().replace(/\s\s+/g, "") == $sarg &&
      !jQuery(this).hasClass('stockout') >= 0
    ) {
      $text = false;
    }
  });
...
return $text;
  ...
  if (jQuery("ul#configurable_swatch_size a").length > 0) {
    [
      jQuery(
        "ul#configurable_swatch_size li.active"
      ).length > 0
        ? jQuery(
            "ul#configurable_swatch_size li.active"
          )
        ...
        : "Select Size",
      jQuery.makeArray(
        jQuery("ul#configurable_swatch_size a").map(function (
          i,
          e
        ) {
          if (jQuery(e)  .text()
  ...
  if (
    jQuery("ul#configurable_swatch_size a").length > 0 &&
    $sarg != "Select Size" &&
    $sarg != "No Size"
  ) {
    jQuery("ul#configurable_swatch_size a").each(function () {
      if (jQuery(e).text()
      .trim()
      .replace(/\s\s+/g, "") == $sarg) jQuery(this).click();
    });
  }
  wait_for(function () {
    return true;
...
// Select
if (jQuery("select:first").length > 0) {
  [
    jQuery("select:first option:selected").text() != ""
      ? jQuery("select:first option:selected")
          .text()
          .trim()
          .replace(/\s\s+/g, "")
      : "Select width",
    jQuery.makeArray(
      jQuery("select:first option").map(function (i, e) {
        if (jQuery(e).text() != "")
          return jQuery(e).text().trim().replace(/\s\s+/g, "");
      })
    ),
...
//pa_sizes clicker
if (
jQuery("select:first").length > 0 &&
$sarg != "Select width" &&
$sarg != "No width"
) {
jQuery("select:first option").each(function () {
  if (jQuery(this).text().trim().replace(/\s\s+/g, "") == $sarg) {
    jQuery("select:first")
      .text(jQuery(this).text())
      .trigger("change");
...
$text = false;
if (
jQuery("select[title="Select book sleeve size"] option").length > 0 &&
$sarg != "No Size" &&
$sarg != "Select Size"
) {
$text = true;
jQuery("select[title="Select book sleeve size"] option").each(function (index) {
  if (
    jQuery(this).text().trim().replace(/\s\s+/g, "") == $sarg &&
    !jQuery(this).attr('disabled')
  ) {
...
if (jQuery("div.product-form--variants .radios--root:contains(Size) input").length > 0) {
  ;[
    jQuery("div.product-form--variants .radios--root:contains(Size) input:checked").length > 0
      ? jQuery("div.product-form--variants .radios--root:contains(Size) input:checked")
      .attr("value")
      .replace(/\s\s+/g, "")
  : "Select item",
jQuery.makeArray(
  jQuery("div.product-form--variants .radios--root:contains(Size) input").map(function (i, e) {
    if (jQuery(e).attr("value") != "")
      return jQuery(e).attr("value").replace(/\s\s+/g, "")
  })
),
...
$sarg != "Select Size" &&
$sarg != "No Size") {
jQuery("div.product-form--variants .radios--root:contains(Size) input").each(function () {
if (jQuery(this).attr("value").replace(/\s\s+/g, "") == $sarg) {
...
$text = false;
if (
  jQuery("div.product-form--variants .radios--root:contains(Size) input").length > 0 &&
  $sarg != "No Size" &&
  $sarg != "Select Size"
) {
  $text = true;
  jQuery("div.product-form--variants .radios--root:contains(Size) input").each(function (index) {
    if (
...
if (jQuery("select#pa_size option").length > 0) {
  [
    jQuery("select#pa_size option:selected").text() != ""
      ? jQuery("select#pa_size option:selected")
          .text()
          .trim()
          // .replace(/\s\s+/g, "")
      : "Select Size",
    jQuery.makeArray(
      jQuery("select#pa_size option").map(function (i, e) {
        if (jQuery(e).text() != "")
          return jQuery(e).text().trim().replace(/\s\s+/g, "");
      })
    ),
...
if (
  jQuery("select#pa_size option").length > 0 &&
  $sarg != "Select Size" &&
  $sarg != "No Size"
  ) {
  ...
  if (
    ...
    $text = true;
    jQuery(select#pa_size option").each(function (index) {
      if (
        jQuery(this).text().trim().replace(/\s\s+/g, "") == $sarg &&
        !jQuery(this).attr('disabled')
      ) {
...
if (jQuery('select[title="Select book sleeve size"] option').length > 0) {
  [
    jQuery('select[title="Select book sleeve size"] option:selected').text() != ""
      ? jQuery('select[title="Select book sleeve size"] option:selected')
          .text()
          .trim()
          .replace(/\s\s+/g, "")
      : "Select Size",
    jQuery.makeArray(
      jQuery('select[title="Select book sleeve size"] option').map(function (i, e) {
        if (jQuery(e).text() != "")
          return jQuery(e).text().trim().replace(/\s\s+/g, "");
      })
    ),
...
if (
  ...
  jQuery('select[title="Select book sleeve size"] option').each(function () {
    if (jQuery(this).text().trim().replace(/\s\s+/g, "") == $sarg) {
      jQuery(this).trigger("change");
    }
  });
...
if (jQuery("div.form-field:contains(Size) input").length > 0) {
  ;[
    jQuery("div.form-field:contains(Size) input:checked").length > 0
      ? jQuery("div.form-field:contains(Size) input:checked")
      .attr("value")
      .replace(/\s\s+/g, "")
  : "Select item",
jQuery.makeArray(
  jQuery("div.form-field:contains(Size) input").map(function (i, e) {
    if (jQuery(e).attr("value") != "")
      return jQuery(e).attr("value").replace(/\s\s+/g, "")
  })
),
...
$sarg != "Select Size" &&
$sarg != "No Size") {
jQuery("div.form-field:contains(Size) input").each(function () {
if (jQuery(this).attr("value").replace(/\s\s+/g, "") == $sarg) {
  jQuery(this).next()[0].click()
}
})
...
$text = false;
if (
  jQuery("div.form-field:contains(Size) input").length > 0 &&
  $sarg != "No Size" &&
  $sarg != "Select Size"
) {
  $text = true;
  jQuery("div.form-field:contains(Size) input").each(function (index) {
    if (
      jQuery(this).attr("value").replace(/\s\s+/g, "") == $sarg &&
      !jQuery(this).hasClass('soldout')
    ) {
